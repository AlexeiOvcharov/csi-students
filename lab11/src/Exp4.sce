...
for x = [2, 4]
    i = 1;
    for Cp = [0.5*koeff, koeff, 2*koeff]
        ...
        if (x == 4) then
            ...
            transData(size(transData, 1) + 1, :) = [Cp, x_stab, t_p, sigma];
        end
...
writeToFile(path + "data/ex" + string(experiment) + "/TransData.dat", ["Cp", "x", "t_p", "sigma"], transData);
